    ...
    GivensRotationQR& compute(const MatrixType& matrix) {
        ...
        for (Index j = 0; j < cols; ++j) {
            for (Index i = rows - 1; i > j; --i) {
                ...
                // Synchronize the data across processes
                synchronizeRows(i - 1, i);
            }
        }
...
private:
    ...
    // Synchronize rows of matrix R across all processes
    void synchronizeRows(Index i, Index k) {
        int rank;
        MPI_Comm_rank(MPI_COMM_WORLD, &rank);

        // Send the rows of matrix R to all other processes for consistency
        MPI_Bcast(&m_matrixR(i - 1, 0), m_matrixR.cols(), MPI_DOUBLE, 0, MPI_COMM_WORLD);
        MPI_Bcast(&m_matrixR(i, 0), m_matrixR.cols(), MPI_DOUBLE, 0, MPI_COMM_WORLD);
    }
