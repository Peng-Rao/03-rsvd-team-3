...
namespace Eigen {
    ...
    GivensRotationQR& compute(const MatrixType& matrix) {
        ...
        for (Index j = world_rank; j < cols; j += world_size) {
        ...
        MPI_Allreduce(MPI_IN_PLACE, m_matrixR.data(), m_matrixR.size(), MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
        MPI_Allreduce(MPI_IN_PLACE, m_matrixQ.data(), m_matrixQ.size(), MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
